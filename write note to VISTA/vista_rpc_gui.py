...
class VistARPCGUI(tk.Tk):
    ...
    def _select_patient(self, dfn):
        ...
        try:
            ...
            if patient_info and patient_info.get("Name"):
                ...
                self.refresh_encounters_button.config(state=tk.NORMAL)
            else:
    ...
    def _fetch_patient_encounters(self):
        self.log_window.log("Populating encounters with static list for development.")
        self.encounter_tree.delete(*self.encounter_tree.get_children())

        clinics = [
            ("CARDIOLOGY", "100"), ("GENERAL MEDICINE", "101"), ("GENERAL SURGERY", "102"),
            ("LABORATORY", "103"), ("MAMMOGRAM", "104"), ("NEPHROLOGY", "105"),
            ("NUCLEAR MEDICINE", "106"), ("OBSERVATION", "107"), ("PLASTIC SURGERY", "108"),
            ("PODIATRY", "109"), ("PRIMARY CARE", "200"), ("PSYCHIATRY", "110"),
            ("RADIOLOGY", "111"), ("SOCIAL WORK", "112"), ("SURGICAL CLINIC", "113")
        ]

        fm_timestamp = self.vista_client._get_fileman_timestamp()
        for clinic_name, location_ien in clinics:
            visit_str = f"{location_ien};{fm_timestamp};A"
    ...
    def _get_recent_notes_for_current_patient(self):
        ...
        if self.current_dfn:
            ...
            # status = self.note_status_combobox.get()
            # context_map = {"All": 15, "Signed": 1, "Unsigned": 2}
            # context = context_map.get(status, 1)
            context = 15 # Hardcode to "All" to avoid error from missing combobox
    ...
    def _get_unsigned_notes_for_current_patient(self):
        ...
        if self.current_dfn:
            # context=2 is for unsigned notes based on the comment in _get_recent_notes_for_current_patient
            self._fetch_patient_notes(self.current_dfn, 100, 3, context=2)
        else:
            messagebox.showwarning("No Patient Selected", "Please select a patient first.")
    ...
    def _show_alert_list(self):
        ...
        if not self.vista_client.connection:
            ...
            return
        try:
            # Assuming ORQQAL LIST takes no parameters or a patient DFN
            # For now, let's assume it takes no parameters and returns a list of alerts
            alerts = self.vista_client.invoke_rpc("ORQQAL LIST", "")
    ...
    def _create_widgets(self):
        ...
        patient_tab.rowconfigure(4, weight=1)
        ...
        ttk.Label(add_note_tab, text="Encounter Location:").grid(row=3, column=0, padx=5, pady=5, sticky="w")
        self.encounter_location_entry = ttk.Entry(add_note_tab)
        self.encounter_location_entry.grid(row=3, column=1, padx=5, pady=5, sticky="ew")
        self.encounter_location_entry.insert(0, "1") # Default to 1, should be a valid location IEN

        ttk.Label(add_note_tab, text="Electronic Signature:").grid(row=4, column=0, padx=5, pady=5, sticky="w")
        self.es_code_entry = ttk.Entry(add_note_tab, show="*")
        self.es_code_entry.grid(row=4, column=1, padx=5, pady=5, sticky="ew")

        self.save_note_button = ttk.Button(add_note_tab, text="Save Note", command=self._save_note, state=tk.DISABLED)
        self.save_note_button.grid(row=5, column=1, padx=5, pady=10, sticky="e")

        self.save_unsigned_button = ttk.Button(add_note_tab, text="Save Unsigned", command=self._save_note_unsigned, state=tk.DISABLED)
        self.save_unsigned_button.grid(row=5, column=0, padx=5, pady=10, sticky="e")
    ...
    def _save_note_unsigned(self):
        self._save_note(sign=False)

    def _save_note(self, sign=True):
        if not self.vista_client.connection:
            messagebox.showwarning("RPC Error", "Not connected to VistA. Please connect first.")
            return
        ...
        title_ien = self.note_titles.get(self.note_title_combobox.get())
        if not title_ien:
            messagebox.showwarning("Input Error", "Please select a note title.")
            return

        note_content = self.note_content_text.get(1.0, tk.END).strip()
        if not note_content:
            messagebox.showwarning("Input Error", "Note content cannot be empty.")
            return

        es_code = self.es_code_entry.get()
        if sign and not es_code:
            messagebox.showwarning("Input Error", "Electronic signature is required.")
            return
            
        encounter_location_ien = self.encounter_location_entry.get()
        if not encounter_location_ien:
            messagebox.showwarning("Input Error", "Encounter location is required.")
            return

        selected_encounter = self.encounter_tree.selection()
        if selected_encounter:
            visit_str = selected_encounter[0]
        else:
            self.log_window.log("No encounter selected. Using a default encounter string for development.")
            # Format: LocationIEN;DateTime;Purpose (A=Ambulatory)
            default_location = self.encounter_location_entry.get() or "200" # Default to PRIMARY CARE IEN
            fm_timestamp = self.vista_client._get_fileman_timestamp()
            visit_str = f"{default_location};{fm_timestamp};A"

        try:
            encounter_datetime = self.vista_client._get_fileman_timestamp()
            result = self.vista_client.create_note(
                self.current_dfn,
                title_ien,
                note_content,
                encounter_location_ien,
                encounter_datetime,
                visit_str,
                es_code if sign else None,
                sign_note=sign
            )
            messagebox.showinfo("Success", result)
            self.note_content_text.delete(1.0, tk.END)
            self.es_code_entry.delete(0, tk.END)
            time.sleep(1)
            self._fetch_patient_notes(self.current_dfn, 100, 3, context=2)
        except Exception as e:
            self.log_window.log(f"Failed to save note: {e}")
            messagebox.showerror("RPC Error", f"Failed to save note: {e}")

    def _load_note_titles(self):
        try:
            self.log_window.log("Loading all note titles...")
            # Fetch all titles by calling with appropriate parameters
            titles = self.vista_client.get_note_titles(doc_class_ien=3, start_from="", direction=1)
            self.note_titles = {item['Title']: item['IEN'] for item in titles}
            self.note_title_combobox['values'] = list(self.note_titles.keys())
            self.log_window.log("All note titles loaded successfully.")
        except Exception as e:
            self.log_window.log(f"Failed to load note titles: {e}")
    ...
    def _connect_to_vista(self):
        ...
        try:
            ...
            self.get_unsigned_notes_button.config(state=tk.NORMAL) # Enable new button
            self.show_alert_list_button.config(state=tk.NORMAL) # Enable new button
